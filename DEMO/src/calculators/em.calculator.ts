...
export function calculateWithEM(
  ...
  const concurrentUsers =
    typeof emSet.concurrent_users?.value === 'number' && emSet.concurrent_users.value > 0
      ? emSet.concurrent_users.value
      : Math.max(1, Math.ceil(userCount * 0.1));
  const userBuckets = Math.max(1, Math.ceil(concurrentUsers / 100));
  const moduleEstimatedStorageGB = matchedCatalogEntries.reduce(
    (sum, x) => sum + (x.catalog?.storageQuotaGBPerUser || 0) * userCount,
    0
  );
  const expectedStorageGB =
    typeof emSet.expected_storage_gb?.value === 'number' && emSet.expected_storage_gb.value > 0
  ...
  const rawLicenseCostFromModules = matchedCatalogEntries.reduce(
    (sum, x) => sum + (x.catalog?.licenseCostBase || 0),
    0
  );
  ...
  const costLineItems: CostLineItem[] = [
    ...
    },
    {
      ...
      amount: serverCost,
      explanation:
        `Base infra + compute by concurrent users + storage usage${requiresHA ? ' + high availability uplift' : ''}` +
      ...
      components: [
        ...
        },
        {
          ...
          citation:
            emSet.concurrent_users?.evidence ||
        ...
        },
        {
          ...
          citation:
            emSet.expected_storage_gb?.evidence ||
