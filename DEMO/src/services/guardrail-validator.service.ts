import { GuardrailValidationOutput, NegotiationCard } from '../types/negotiation.types';
...
export class GuardrailValidatorService {
  validate(input: GuardrailValidationInput): GuardrailValidationOutput {
    ...
    for (const card of input.selectedCards) {
      if (card.blocksIfMissingModules && card.blocksIfMissingModules.length > 0) {
        const hasRequiredModule = card.blocksIfMissingModules.some((required) =>
          normalizedModuleIds.some((moduleId) => this.moduleMatchesRequirement(moduleId, required))
        );

        if (!hasRequiredModule) {
          dependencyPass = false;
          warnings.push(
            `Dependency: card ${card.cardId} yeu cau module ${card.blocksIfMissingModules.join(', ')} nhung scope hien tai khong co.`
          );
        }
      }

      const missingCapabilities = card.requiresClientCapability.filter(
        (cap) => !capabilitySet.has(cap.toLowerCase())
      );

      if (missingCapabilities.length > 0) {
        operationPass = false;
        warnings.push(
          `Operation: card ${card.cardId} thieu capability ${missingCapabilities.join(', ')} tu phia khach hang.`
        );
      }
    }
    ...
    if (!pricingIntegrityPass) {
      warnings.push(
        `Pricing integrity: gia sau dieu chinh (${postDiscountPriceVnd.toLocaleString('vi-VN')} VND) thap hon floor (${pricingFloorVnd.toLocaleString('vi-VN')} VND).`
      );
    }
  ...
  private moduleMatchesRequirement(moduleId: string, requirement: string): boolean {
