...
async function callGemini(prompt) {
    ...
    if (!apiKey)
        throw new Error('[KHantix] GEMINI_API_KEY is not set in .env');
...
async function callOpenAI(prompt) {
    ...
    if (!apiKey)
        throw new Error('[KHantix] OPENAI_API_KEY is not set in .env');
...
async function callAnthropic(prompt) {
    ...
    if (!apiKey)
        throw new Error('[KHantix] ANTHROPIC_API_KEY is not set in .env');
...
function createLlmCaller() {
    ...
    switch (provider) {
        ...
        case '9router':
            ...
            return call9Router;
        default:
            throw new Error(`[KHantix] Unknown LLM_PROVIDER="${provider}". ` +
