...
exports.override = exports.report = exports.calculate = exports.baseReport = void 0;
...
const baseReport = (req, res) => {
    const sessionId = req.query.sessionId;
    if (!sessionId) {
        return res.status(400).json({ error: 'sessionId query param is required' });
    }
    const userCountOverride = req.query.userCount ? parseInt(req.query.userCount, 10) : null;
    const concurrentUsersOverride = req.query.concurrent_users ? parseInt(req.query.concurrent_users, 10) : null;
    const expectedStorageGBOverride = req.query.expected_storage_gb ? parseFloat(req.query.expected_storage_gb) : null;
    const requiresHAOverride = req.query.requires_high_availability !== undefined
        ? String(req.query.requires_high_availability).toLowerCase() === 'true'
        : null;
    let roleAllocationOverride = null;
    if (req.query.roleAllocation) {
        try {
            roleAllocationOverride = JSON.parse(req.query.roleAllocation);
        }
        catch {
            roleAllocationOverride = null;
        }
    }
    else if (req.body?.roleAllocation) {
        roleAllocationOverride = req.body.roleAllocation;
    }
    ...
    const effectiveEMSet = {
        ...emSet,
        concurrent_users: concurrentUsersOverride !== null && !Number.isNaN(concurrentUsersOverride)
            ? {
        ...
        expected_storage_gb: expectedStorageGBOverride !== null && !Number.isNaN(expectedStorageGBOverride)
            ? {
        ...
        requires_high_availability: requiresHAOverride !== null
            ? {
    ...
    const calcResult = (0, em_calculator_1.calculateWithEM)({
...
const report = (req, res) => {
    ...
    const concurrentUsers = req.query.concurrent_users ? parseInt(req.query.concurrent_users, 10) : undefined;
    const expectedStorageGB = req.query.expected_storage_gb ? parseFloat(req.query.expected_storage_gb) : undefined;
    const requiresHA = req.query.requires_high_availability !== undefined
        ? String(req.query.requires_high_availability).toLowerCase() === 'true'
        : undefined;
    // Fallbacks for query params if roleAllocation is passed
    const roleAllocationQuery = req.query.roleAllocation ? JSON.parse(req.query.roleAllocation) : undefined;
    if (!sessionId) {
        return res.status(400).json({ error: 'sessionId is required' });
    }
    const serverSession = session_repository_1.sessionRepository.get(sessionId);
    ...
    const effectiveEMSet = {
        ...
        concurrent_users: concurrentUsers !== undefined
            ? {
        ...
        expected_storage_gb: expectedStorageGB !== undefined
            ? {
        ...
        requires_high_availability: requiresHA !== undefined
            ? {
                value: requiresHA,
                confidence: 'high',
                is_extracted: true,
